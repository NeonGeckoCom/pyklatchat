...
secret_key = cookies_config.get('SECRET', "")
...
def check_password_strength(password: str) -> str:
    """
        ...
        :returns: 'OK' if input string is strong enough,
            unfulfilled condition otherwise
    """
    if len(password) < 8:
...
def create_unauthorized_user(response: Response, authorize: bool = True,
                             nano_token: str = None) -> dict:
    """
    ...
    LOG.debug(f"Generated guest nick: {guest_nickname}")
    if nano_token:
        ...
        new_user = create_from_pattern(
            ...
            override_defaults=dict(nickname=guest_nickname))
    LOG.debug(f"Inserting new_user: {new_user}")
    db_controller.exec_query(query={'document': 'users',
                                    'command': 'insert_one',
                                    'data': new_user})
    if authorize:
        LOG.debug(f"Authorizing new_user: {new_user}")
        token = jwt.encode({"sub": new_user['_id'],
                            'creation_time': time(),
                            'last_refresh_time': time()}, secret_key)
        LOG.debug(f"Adding session token: {token}")
        response.set_cookie('session', token, httponly=True)
    LOG.debug(f"Returning new_user: {new_user}")
    return new_user


def get_current_user(request: Request, response: Response,
                     force_tmp: bool = False, nano_token: str = None) -> dict:
    """
        ...
        :returns user id based on received cookies or sets
            temporal user cookies if not found
    """
    user_id = None
    user = {}
    if not force_tmp:
        if nano_token:
            ...
            try:
                ...
                if session:
                    payload = jwt.decode(jwt=session, key=secret_key,
                                         algorithms=jwt_encryption_algo)
                    current_timestamp = time()
                    if (int(current_timestamp) -
                        int(payload.get('creation_time', 0))) <= \
                            cookie_lifetime:
                        user = db_controller.exec_query(
                            query={'command': 'find_one',
                                   'document': 'users',
                        ...
                        if not user:
                            LOG.info(f'{payload["sub"]} is not found in users,'
                                     f' setting temporal user credentials')
                        else:
                            user_id = user['_id']
                            if (int(current_timestamp) -
                                int(payload.get('last_refresh_time', 0))) >= \
                                    cookie_refresh_rate:
                                refresh_cookie(payload=payload,
                                               response=response)
                                LOG.info('Cookie was refreshed')
            except BaseException as ex:
                LOG.warning(f'Problem resolving current user: {ex},'
